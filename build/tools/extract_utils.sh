...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$OCTAVI_ROOT"/vendor/octavi/build/tools/smali/baksmali.jar
        export SMALIJAR="$OCTAVI_ROOT"/vendor/octavi/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$OCTAVI_ROOT"/vendor/octavi/build/tools/"$HOST"/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$OCTAVI_ROOT"/vendor/octavi/build/tools/"$HOST"/compact_dex_converter
    fi
