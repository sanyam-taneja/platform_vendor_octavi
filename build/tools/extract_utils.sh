...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$OCTAVI_ROOT"/prebuilts/tools-extras/common/tools/smali/baksmali.jar
        export SMALIJAR="$OCTAVI_ROOT"/prebuilts/tools-extras/common/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$OCTAVI_ROOT"/prebuilts/tools-extras/"${HOST,,}"-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$OCTAVI_ROOT"/prebuilts/tools-extras/"${HOST,,}"-x86/bin/compact_dex_converter
    fi
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$OCTAVI_ROOT"/vendor/octavi/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
